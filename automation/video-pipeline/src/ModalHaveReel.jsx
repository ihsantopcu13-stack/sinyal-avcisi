...
// ============================================================
// MODAL + HAVE V3 Tuzağı Reel — OF Tuzağı serisinin on üçüncü bölümü,
...
export const SCENE_FRAMES = [88, 102, 62, 130, 113, 35, 30];
...
function Scene2({ frame }) {
  ...
  const boxIn = pop(frame, 55, 14);

  return (
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 24 }}>
        <Word size={28} color={GREEN}>must have V3 → güçlü geçmiş çıkarımı</Word>
        <div style={{ opacity: line2In, transform: `translateY(${(1 - line2In) * 14}px)` }}>
          <Word size={28} color={YELLOW}>should have V3 → geçmişte yapılmalıydı</Word>
        </div>
        <div style={{ opacity: boxIn, transform: `scale(${0.85 + 0.15 * boxIn})` }}>
          <Box color={WHITE} size={38}>HAVE + V3</Box>
        </div>
      </div>
    </AbsoluteFill>
  );
    ...
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 30 }}>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 8, padding: "0 30px", textAlign: "center" }}>
          <Word size={32}>He</Word>
          <Box color={GREEN} size={32}>must have forgotten</Box>
          <Word size={32}>the meeting.</Word>
        </div>
        {flashOn && (
          <div
            style={{
...
function Scene5({ frame }) {
  const l1In = pop(frame, 0, 12);
  const l2In = pop(frame, 12, 12);
  const hitFrame = frame - 27;
    ...
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      {!hitOn ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
          <div style={{ opacity: l1In }}>
            <Box color={YELLOW} size={28}>looks exhausted → ŞİMDİKİ KANIT</Box>
          </div>
          <div style={{ opacity: l2In }}>
            <Box color={GREEN} size={28}>all night → GEÇMİŞ</Box>
          </div>
        </div>
      ) : (
        <div
