...
// ============================================================
// HOWEVER Tuzağı Reel — OF Tuzağı serisinin yedinci bölümü, aynı AVCI
...
export const SCENE_FRAMES = [65, 114, 87, 130, 100, 30, 39];
  ...
  return (
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 22, transform: `translateX(${shake}px)` }}>
        ...
        <div style={{ fontFamily: "sans-serif", fontWeight: 800, fontSize: 32, color: YELLOW, opacity: subOpacity, textAlign: "center", padding: "0 30px", textShadow: `0 0 16px ${YELLOW}` }}>
          ALTHOUGH İLE KARIŞTIRMA!
        </div>
      </div>
    </AbsoluteFill>
  );
...
function Scene2({ frame }) {
  ...
  const dotIn = pop(frame, 45, 14);
  return (
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 26 }}>
        <Word size={34}>ALTHOUGH + CÜMLE, CÜMLE</Word>
        <div style={{ display: "flex", alignItems: "center", gap: 10, opacity: line2In, transform: `translateY(${(1 - line2In) * 14}px)` }}>
          <Box color={YELLOW} size={38}>HOWEVER</Box>
          <Word size={34}>→ YENİ CÜMLE</Word>
        </div>
        <div style={{ opacity: dotIn, transform: `scale(${0.8 + 0.2 * dotIn})` }}>
          <Box color={YELLOW} size={42}>. HOWEVER,</Box>
        </div>
      </div>
    </AbsoluteFill>
  );
...
function Scene3({ frame }) {
  const stage = frame < 25 ? 0 : frame < 45 ? 1 : 2;
  const okIn = pop(frame, 65, 14);
  return (
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 34 }}>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 8, padding: "0 30px", textAlign: "center" }}>
          {stage === 0 ? (
            <Box color={GREEN} size={34}>The method is effective.</Box>
          ) : (
            <Word size={32}>The method is effective.</Word>
          )}{" "}
          {stage === 1 ? (
            <Box color={YELLOW} size={34}>However,</Box>
          ) : (
            <Word size={32} color={stage === 2 ? WHITE : "#666"}>However,</Word>
          )}{" "}
          {stage === 2 ? (
            <Box color={GREEN} size={34}>it is expensive.</Box>
          ) : (
            <Word size={32} color="#666">it is expensive.</Word>
          )}
        </div>
        <div
          style={{
    ...
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      ...
      >
        <div style={{ fontFamily: "sans-serif", fontWeight: 800, fontSize: 28, color: WHITE, textAlign: "center" }}>
          The new technology is useful. _____, it is very expensive.
        </div>
...
function Scene5({ frame }) {
  ...
  const flashOn = flashFrame >= 0 && frame < 33;
  const hitFrame = frame - 33;
    ...
    <AbsoluteFill style={{ background: BG, justifyContent: "center", alignItems: "center" }}>
      {!flashOn && !hitOn && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, opacity: lockOpacity }}>
          ...
          <Box color={YELLOW} size={38}>. _____, it</Box>
        </div>
      )}
      {flashOn && (
        <div
