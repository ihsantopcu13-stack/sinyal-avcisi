...
// ============================================================
{
  ...
  function sandboxKur({ sbImpl, fetchImpl } = {}) {
    ...
    const sandbox = {
      ...
      // KATMAN 5H/5I (canonical next-question / signal-targeted practice
      // köprüleri) bu dosyanın kapsamı DIŞINDA — burada SADECE auth header
      // davranışı test ediliyor, komutlar hiçbir zaman eşleşmiyor (sabit
      // "test mesajı" girdisi) ve _aktifSoruModulu hiç 'sinyal_lab'
      // olmuyor, dnavChat() normal (auth odaklı) akışına HİÇ etkilenmeden
      // devam ediyor.
